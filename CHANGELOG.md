...
# V3
- **added**: **Design Studio V3** — a simplified editor for the agents built with it: one action per block, no action dragging and no inline edit/copy/delete controls, "Add action" only where it belongs, and the block moved by grabbing the action's header, which now needs a few pixels of movement so an imprecise click no longer drags it. Restyled block, without the dead space and the double frame around the single action. Agents built with the previous editor are left exactly as they were
- **added**: the editor an agent opens with is decided **only by the version the agent declares** when it is created, and the creation date is no longer taken into account. Agents that carry no version, which is every agent created so far, open in the previous editor. The version an agent declares is set per environment, separately from the product version
- **changed**: the block's outgoing dot now sits inside the block and is shown only where it really is the block's only way out: it is hidden when the action already has its own connectors (Success/Else, buttons, noInput/noMatch) and on the blocks that end the conversation — close, handoff to an operator, back to the queue, hand over to another agent, and change department when it starts the department's bot. The Reply family always keeps it, so the flow can carry on after a message, and a dot that is already connected is never hidden
- **added**: **create an agent with AI** — a dialog where the agent is described, with a gallery of 21 use cases, a choice of AI model among those available, and an interview that fills in the request one question at a time, showing what is still missing. It ends with an editable final prompt and a preview of the flow, blocks and connections included. The agent it produces is always a new one and never touches the open agent; the interview survives in the browser tab until the agent is created
- **added**: **the agent's AI panel** — inside every V3 agent, a right-hand panel holds the agent's history: the prompt it was created from, the changes made with AI and the restores. A change can be asked for in plain language, is previewed block by block and is applied only on confirmation. Versions and releases are listed with save, restore, copy and reuse of the prompt in the generator; every restore is preceded by an automatic save and warns when that version uses departments, knowledge bases, tables or agents the project no longer has. What is online does not change until it is published again
- **changed**: the agents the AI produces come out more compact and lean on the AI blocks — one knowledge base search instead of a block per question, one AI routing instead of a tree of conditions, an open question instead of a deep menu. Their messages use the simple reply instead of the advanced one, questions and button menus included, and the conversation is no longer closed at the end of a path, so the last message stays readable
- **added**: the generator covers 15 more actions, from random reply and wait to data tables, loops over a list, email, tags, contact update and AI routing, and skips the ones the project's plan or data do not allow. It takes up to three verified agents similar to the request as a reference, and the preview can be rated with a thumb
- **added**: an **agent selector in the header** to move between the project's agents without leaving the editor, and **delete agent** with an explicit confirmation, both only on V3 agents
