...
export class CDSVoiceSettingsComponent implements OnInit {
  ...
  get selectedPreviewUrl(): string | null {
    ...
    return selectedVoice?.preview_url ?? null;
  }

  get selectedElement(): object | null {
    return this.voice_name_list?.find(voice => voice.voiceId === this.voice_name) ?? null;
  }
  ...
  private saveAttributes() {
    ...
    this.faqKbService.addNodeToChatbotAttributes(this.selectedChatbot._id, 'globals', data).subscribe((resp) => {
      this.selectedChatbot.attributes = { globals: data };
      variableList.find(el => el.key === 'globals').elements = this.selectedChatbot.attributes.globals.map(({ key: name, ...rest }) => ({ name, value: name }));
    }, (error) => {
