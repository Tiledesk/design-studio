export class TiledeskStage {
    ...
    /** Pending moved-and-scaled payload; overwritten until rAF fires. */
    pendingMovedAndScaled = null;
    /** True when a requestAnimationFrame is scheduled to dispatch moved-and-scaled. */
    rafScheduledMovedAndScaled = false;
    ...
    constructor(containerId, drawerId, classDraggable) {
        ...
        this.scheduleDispatchMovedAndScaled = this.scheduleDispatchMovedAndScaled.bind(this);
    }

    /**
     * Schedules at most one "moved-and-scaled" dispatch per animation frame.
     * Stores latest {x, y, scale}; overwrites pending if called again before rAF.
     */
    scheduleDispatchMovedAndScaled() {
        this.pendingMovedAndScaled = { scale: this.scale, x: this.tx, y: this.ty };
        if (this.rafScheduledMovedAndScaled) return;
        this.rafScheduledMovedAndScaled = true;
        const self = this;
        requestAnimationFrame(function flush() {
            self.rafScheduledMovedAndScaled = false;
            const p = self.pendingMovedAndScaled;
            self.pendingMovedAndScaled = null;
            if (p) {
                const customEvent = new CustomEvent("moved-and-scaled", { detail: { scale: p.scale, x: p.x, y: p.y } });
            ...
            if (event.button === 1) { 
                ...
                document.onmousemove = (function(event) {
                    if (isDragging) {
                        ...
                        this.transform();
                        this.scheduleDispatchMovedAndScaled();
                    }
                }).bind(this);
    ...
    moveAndZoom(event) {
        ...
        // Do not call getPositionNow() here: tx/ty/scale are source of truth; parsing getComputedStyle every wheel is expensive.
